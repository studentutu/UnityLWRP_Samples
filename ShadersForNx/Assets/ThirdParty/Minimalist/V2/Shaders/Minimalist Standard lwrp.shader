...
// 							void MainLightWithShadows (float3 WorldPos, out float3 Direction, out float3 Color, out float DistanceAtten, out float ShadowAtten)
// 							{
...
// 								   Light mainLight = GetMainLight(shadowCoord); // GetMainLight(shadowCoord) in Fragment!
...
//                  			{
//                  				vertexOutput o;
// 								//float4 veroPos =  float4(v.vertex.xyz, 1.0);
// 								//float4x4 mpmult = GetVertexPositionInputs(v.vertex.xyz); //mul(UNITY_MATRIX_MV, veroPos);
...
// 									MainLightWithShadows(o.worldPos, Direction, Color, DistanceAtten, ShadowAtten);
// 									float shadAtt = DistanceAtten*ShadowAtten;
// 									float3 multiplied = float3(shadAtt,shadAtt,shadAtt);
// 									Color = multiplied* Color;
// 									//o.diff = lerp(o.diff , o.customLighting, Color.a);
// 									o.diff  = float4(Color.rgb,1);

// 									//o.diff = Maincolor * ;// * _LightColor0.rgb;
// 									//o.ambient = half4( ShadeSH9(half4(worldNormal,1)).rgb, 1);
//                  				#endif
//                  				//Apply Unity fog
//                  				#if UNITY_FOG
...
// 									// SHADOW INTENCITY BY ALPHA COLOR
// 									fixed4 normalShadow =  Result * fixed4(i.diff * _ShadowColor.a);
// 									Result = lerp(Result, normalShadow, _ShadowColor.a);
//                  				#endif
