...
#define LPW_AMBIENT(normal)  max(0,ShadeSH9(half4(normal, 1)))  // ShadeSH9 returns approximate value, it shoud never be negative!
...
float ripple(float2 p, float phase)
{
...
half4 lighting(half3 normal, half3 worldPos)
{
	...
	fixed3 result = _Color*ambient + LIGHTCOLOR * (fres + spec);
	...
	// //diffuse lighting
	// half3 diff = _Color *saturate(dot(normal,lightDir));
	// // half3 diff = _Color*max(0.0, dot (normal, lightDir));
	...
	// //specular
	// // half3 h = normalize (lightDir + worldViewDir);
	// // half nh =  dot (normal, h);
	// half specPower =  _Gloss;
	// half3 spec = _SpecColor.rgb*specPower;

	
	// spec = _Color*ambient + LIGHTCOLOR * (fres + spec);
	// ambient = step(spec.r + spec.g + spec.b, 0.45);  // arg[1] > arg[0]

	// spec = lerp( spec, _Color,ambient );
	// return half4(
	// 	/*rgb:  */   spec, // LIGHTCOLOR  _Color*0.1 + _Color*ambient*0.9 +  LIGHTCOLOR * (2*fres + spec)
	// 	/*alpha:*/1);  // _Opacity*(1.0+0.2*fresPower+specPower)
}
...
v2f vert (appdata v) 
{
	...
	// 	o.vertexLight = lighting(worldNormal, pos0.xyz);
	// #else // flat shading
	// 	o.vertexLight = lighting(worldNormal,  pos0.xyz ); //  (pos0.xyz+pos1+pos2)/3.0);
	// #endif
